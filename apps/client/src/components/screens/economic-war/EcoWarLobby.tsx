    ...
    <div className="space-y-5">
      ...
      {game.connectionInfo.isHost && (
        <div className="rounded-xl border border-slate-200/80 dark:border-slate-700 bg-white/90 dark:bg-slate-800/90 p-4 space-y-3">
          ...
          <ConfigSlider label="Timer action (s)" value={config.actionTimerSeconds} min={30} max={180} step={10}
            onChange={(v) => handleConfigChange({ actionTimerSeconds: v })} />
          <ConfigSlider label="Timer draft (s)" value={config.draftTimerSeconds} min={15} max={60} step={5}
